...
void generate_from_file(int argc, char *argv[]) {
    ...
    i64 len = file_information.st_size;
    u8 *string = (u8 *)mmap(NULL, len, PROT_READ, MAP_PRIVATE, input_fd, 0);
    ...
    i64 err = libsais64(string, sa, len, 0, NULL);
    if (err < 0) {
        printf("sa error\n");
        free(sa);
        munmap(string, len);
    ...
    i64 *plcp = arralloc(i64, len);
    err = libsais64_plcp(string, sa, plcp, len);
    munmap(string, len);
    if (err < 0) {
        printf("plcp error\n");
        free(sa);
